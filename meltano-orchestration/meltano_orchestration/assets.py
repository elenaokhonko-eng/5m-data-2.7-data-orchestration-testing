...
def pipeline_meltano(context: AssetExecutionContext, pipes_subprocess_client: PipesSubprocessClient):
    ...
    return pipes_subprocess_client.run(
        ...
        context=context,
        cwd = "\\wsl.localhost\Ubuntu\home\dancer\Projects\5m-data-2.6-data-pipelines-orchestration\meltano-resale"
    ).get_results()
...
def pipeline_dbt_run(context: AssetExecutionContext, pipes_subprocess_client: PipesSubprocessClient):
    ...
    return pipes_subprocess_client.run(
        command=["dbt", "build"],
        context=context,
        cwd = "\\wsl.localhost\Ubuntu\home\dancer\Projects\5m-data-2.6-data-pipelines-orchestration\resale_flat"
    ).get_results()
